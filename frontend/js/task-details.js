...
function renderTaskDetails(task) {
  ...
  if (!isOwn && task.status === "Posted") {
    ...
    actionArea = `<button class="primary-button" id="startTaskButton" data-task-id="${task.id}">
                    <i class="ti ti-player-play" aria-hidden="true"></i> Start Task
                  </button>`;
  } else if (task.status === "In Progress" && isAcceptedByMe) {
    actionArea = `<button class="primary-button" id="completeTaskButton" data-task-id="${task.id}">

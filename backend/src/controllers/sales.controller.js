...
async function createSalesItem(req, res, next) {
  try {
    ...
    return res.status(201).json({
      success: true,
      message: "Sales item listed successfully.",
      data: item
    });
  } catch (error) {
    next(error);
...
async function getMySalesItems(req, res, next) {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "My sales items fetched successfully.",
      data: items
    });
  } catch (error) {
    next(error);
...
async function markSalesItemAsSold(req, res, next) {
  try {
    ...
    const item = await salesService.markSalesItemAsSold(
    ...
    return res.status(200).json({
      success: true,
      message: "Sales item marked as sold.",
      data: item
    });
  } catch (error) {
    next(error);

...
async function attachLatestEndorsements(items, contextType) {
  ...
  try {
    ...
    /* Never let a broken endorsements table take down the whole listing
       page — log it loudly so it's visible in the terminal, but return
       the items unendorsed rather than failing the request. */
    console.error(`attachLatestEndorsements failed for contextType="${contextType}":`, err.message);
    return items;
  }
}
